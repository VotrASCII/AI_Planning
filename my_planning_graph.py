...
class PgNode_a(PgNode):
    ...
    def __init__(self, action: Action):
        ...
        PgNode.__init__(self)
        self.action = action
        self.prenodes = self.precond_s_nodes()     # parent nodes
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        :return:
            adds A nodes to the current level in self.a_levels[level] # won't return, just adds...
        """
        ...
        # PgNode_a; s_level; prenodes

        l_node = []
        for action in self.all_actions:
            a_node = PgNode_a(action)  # 1 created action node
            if a_node.prenodes.issubset(self.s_levels[level]):  # if subset available in parent node for this particular level
                for s_node in self.s_levels[level]:     # 2
                    a_node.parents.add(s_node)  # 1 actions to add to previous S literal
                    s_node.children.add(a_node)     # 2 connecting action to previous action literal
                l_node.append(a_node)   # actions operating at a particular level
        l_node = set(l_node)
        self.a_levels.append(l_node)
    ...
    def add_literal_level(self, level):
        ...
        l_node = []  # level
        a_level = self.a_levels[level-1]    # immediate previous level action as effect for current literal
        for action_node in a_level:
            for add_e in action_node.action.effect_add:     # like Eaten(Cake)
                literal_node = PgNode_s(add_e, True)  # current node representation
                action_node.children.add(literal_node)  # subsequent action node... Si then Ai
                l_node.append(literal_node)

            # e.g. Eaten(Cake) and Not Eaten(Cake) both should be represented for its contribution to action from literal

            for remove in action_node.action.effect_rem:    # like Not Eaten(Cake)
                literal_node = PgNode_s(remove, False)  # shouldn't contribute in next for this particular action_node
                action_node.children.add(literal_node)
                l_node.append(literal_node)
        l_node = set(l_node)
        self.s_levels.append(l_node)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # precond_pos/neg negated by effect_rem/add
        if (set(node_a1.action.precond_pos) and set(node_a2.action.effect_rem)) or \
                (set(node_a1.action.precond_neg) and set(node_a2.action.effect_add)) or \
                (set(node_a2.action.precond_pos) and set(node_a1.action.effect_rem)) or \
                (set(node_a2.action.precond_neg) and set(node_a1.action.effect_add)):
            return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for literal_1 in node_a1.parents:
            for literal_2 in node_a2.parents:
                if literal_2.is_mutex(literal_1):
                    return True

        if (set(node_a1.action.precond_pos) and set(node_a2.action.precond_neg)) or \
                (set(node_a1.action.precond_neg) and set(node_a2.action.precond_pos)):
            return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for action_1 in node_s1.parents:
            for action_2 in node_s2.parents:
                if not action_2.is_mutex(action_1):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        remaining = set([goal for goal in goals])
        level = 0
        while level < len(self.s_levels) and remaining:
            for literal in self.s_levels[level]:
                if literal.symbol in remaining:
                    if literal.is_pos:
                        level_sum += level
                        remaining.remove(literal.symbol)
            level += 1
        return level_sum
